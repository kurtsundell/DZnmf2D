function [PDP,Weightings,finalResidual, coefficient_count,R2, numIter]...
    =DZnmf_loop(Basin_PDP, nsources, option)
...
for i=2:nsources
	if cancel == 1
		...
		iter_num = i;
		[PDP(:,:,i-1),Weightings(:,:,i-1),numIter(i-1),tElapsed,finalResidual(i-1)]=DZnmf(Basin_PDP,i, option);
